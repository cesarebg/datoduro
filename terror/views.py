...
# Did all the other functions outside the app and I just plugged the data for
# better management at the server
def index(request):
    template = loader.get_template('terror/index.html')
    file = open("terror/templates/terror/convert.txt", "r")
    contents = file.read()
    context = ast.literal_eval(contents)
    file.close()
